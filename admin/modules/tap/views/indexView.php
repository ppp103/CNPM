<?php 
    // $list_data_Tap = show_Tap();
    // foreach($list_data_Tap as &$data){
    //     $data['btn_suaTap'] = "?mod=tap&action=update&id={$data['id']}";
    //     $data['btn_delete'] = "?mod=tap&action=xoa&id={$data['id']}";
    // }
    // unset($data);
?>
...
<div class="main-content">
    <div class="page-header">
        <h2 class="header-title">Danh Sách Tập Sách </h2> 
    </div>
    <div class="card">
        <div class="card-body">
            <div class="row m-b-30">
                ...
                <div class="col-lg-4 text-right">
                    <button class="btn btn-primary">
                        ...
                        <a href="?mod=tap&action=them" style="color:white;">Thêm Tập</a>
                        <span></span>
                    </button>
                </div>
            </div>
            <div class="table-responsive">
                <table class="table table-hover e-commerce-table">
                    ...
                    <?php 
                        ...
                        $itemsPerPage = 10;
                        $paginationResult = paginate("tap", $currentPage, $itemsPerPage);
                        
                        // Hiển thị dữ liệu
                        foreach ($paginationResult['data'] as $data) {
                            $list_data_Sach = show_Sach($data['sach_id']);
                            $data['btn_suaTap'] = "?mod=tap&action=update&id={$data['id']}";
                            $data['btn_delete'] = "?mod=tap&action=xoa&id={$data['id']}";
                    ?>
                        <tr>
                            ...
                            <td>
                                <div class="d-flex align-items-center">
                                    <img class="img-fluid rounded" src="public/images/tap/<?php echo $data['anh']?>" style="max-width: 60px" alt="">
                                </div>
                            </td>
                            <td>
                                <div class="d-flex align-items-center">                           
                                    <h6 class="m-b-0 m-l-10"><?php echo $data['soTap'] ?></h6>
                                </div>
                            </td>
                            <td><?php echo $data['noiDung'] ?></td>
                            <td><?php echo $list_data_Sach['tenSach'] ?></td>    
                            
                            <td class="text-right">
                                <a href="<?php echo $data['btn_suaTap'] ?>" class="btn btn-icon btn-hover btn-sm btn-rounded pull-right">
            ...
            <nav aria-label="Page navigation example">
                <ul class="pagination justify-content-center">
                    ...
                    for ($i = 1; $i <= $paginationResult['totalPages']; $i++) {
                        ...
                        echo "<li class='page-item $isActive'><a class='page-link' href='?mod=tap&action=index&page=$i'>$i</a></li>";
                    }
                    ?>
                </ul>
            </nav>
...
$(document).ready(function(){
    $('#sort-select').change(function() {
        ...
        rows.sort(function(a, b) {
            var keyA = $(a).children('td').eq(4).text(); // Tên đăng nhập của hàng a
            var keyB = $(b).children('td').eq(4).text(); // Tên đăng nhập của hàng b
